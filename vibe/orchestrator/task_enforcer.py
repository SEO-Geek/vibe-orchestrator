...
INTENT_PATTERNS: dict[str, list[tuple[str, float]]] = {
    ...
    "research": [
        ...
        # Question patterns - "what/where/which/who/when/how" + noun
        (r"^what\s+(files?|functions?|classes?|methods?|modules?|components?)\s+", 0.90),
        (r"^where\s+(is|are|does|do|can)\s+", 0.90),
        (r"^which\s+(files?|functions?|classes?|methods?|modules?)\s+", 0.90),
        (r"^who\s+(wrote|created|maintains|owns)", 0.85),
        (r"^when\s+(was|did|does)", 0.85),
        (r"^how\s+(does|do|is|are|many|much)\s+", 0.85),
        (r"^why\s+(does|do|is|are)\s+.*\?$", 0.80),
        # Investigation commands - find/list/search/locate/show
        (r"^(find|locate)\s+(all|the|where|which|what)", 0.90),
        (r"^(list|show)\s+(all|the|every|each)\s+", 0.90),
        (r"^search\s+(for|the|in)\s+", 0.85),
        (r"(find|list|show)\s+.*\s+(files?|functions?|classes?|usages?|references?)", 0.85),
        # Explanation requests
        (r"^explain\s+(how|the|this|what)", 0.90),
        (r"^describe\s+(how|the|this|what)", 0.85),
        (r"^(tell|walk)\s+me\s+(about|through)", 0.85),
        # Code understanding patterns
        (r"what\s+does\s+.*\s+(do|mean|return)", 0.90),
        (r"how\s+does\s+.*\s+work", 0.90),
        (r"(handles?|responsible\s+for|manages?)\s+.*\?$", 0.80),
        # Generic question ending with "?"
        (r"^(what|where|which|how|why)\s+.*\?$", 0.75),
    ],
    "ui_test": [
